...
import com.linkedin.pinot.core.io.compression.ChunkUncompressor;
...
public class VarByteSingleValueReader extends BaseSingleColumnSingleValueReader<VarByteReaderContext> {
  ...
  private final ChunkUncompressor _chunkUncompressor;
   ...
   * @throws IOException
   */
  public VarByteSingleValueReader(PinotDataBuffer pinotDataBuffer, ChunkUncompressor uncompressor)
      throws IOException {
    _chunkUncompressor = uncompressor;
    ...
    int maxLength = _dataBuffer.getInt(headerOffset);
    headerOffset += INT_SIZE;

    int chunkHeaderSize = _numDocsPerChunk * INT_SIZE;
    _maxChunkSize = chunkHeaderSize + (maxLength * _numDocsPerChunk);
    ...
    _reusableBytes.set(new byte[maxLength]);
  }
  ...
  private ByteBuffer getChunkForRow(int row, VarByteReaderContext context)
    ...
    _chunkUncompressor.uncompress(_dataBuffer.toDirectByteBuffer(chunkPosition, chunkSize), uncompressedBuffer);
